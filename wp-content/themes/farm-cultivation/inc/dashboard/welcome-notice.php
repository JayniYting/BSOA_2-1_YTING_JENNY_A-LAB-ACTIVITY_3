 ...
 */
if (!function_exists('farm_cultivation_is_plugin_installed')) {
    function farm_cultivation_is_plugin_installed($plugin_slug)
    {
        $plugin_path = WP_PLUGIN_DIR . '/' . $plugin_slug;
        return file_exists($plugin_path);
    }
}
if (!function_exists('farm_cultivation_is_plugin_activated')) {
    function farm_cultivation_is_plugin_activated($plugin_slug)
    {
...
add_action('wp_ajax_farm_cultivation_install_and_activate_plugins', 'farm_cultivation_install_and_activate_plugins');
add_action('wp_ajax_nopriv_farm_cultivation_install_and_activate_plugins', 'farm_cultivation_install_and_activate_plugins');
add_action('wp_ajax_farm_cultivation_rplugin_activation', 'farm_cultivation_rplugin_activation');
add_action('wp_ajax_nopriv_farm_cultivation_rplugin_activation', 'farm_cultivation_rplugin_activation');
...
function farm_cultivation_all_plugins_active() {
...
function farm_cultivation_install_and_activate_plugins() {
    if (!current_user_can('manage_options')) {
        return;
    }
    check_ajax_referer('farm_cultivation_welcome_nonce', 'nonce');
    ...
    foreach ($recommended_plugins as $plugin) {
        ...
        if (is_farm_cultivation_plugin_installed($plugin_slug)) {
    ...
    $redirect_url = admin_url('themes.php?page=farm-cultivation-guide-page');
    echo json_encode([
        'success' => true,
        'data' => [
...
function is_farm_cultivation_plugin_installed($plugin_slug) {
...
add_action('wp_ajax_farm_cultivation_dismissed_notice_handler', 'farm_cultivation_ajax_notice_dismiss_function');

function farm_cultivation_ajax_notice_dismiss_function() {
    if (!wp_verify_nonce($_POST['wpnonce'], 'farm_cultivation_welcome_nonce')) {
...
function farm_cultivation_custom_admin_notice() {
    if (!get_option('dismissed-get_started_notice', false)) {
        $farm_cultivation_current_screen = get_current_screen();
        $farm_cultivation_theme = wp_get_theme();
        if ($farm_cultivation_current_screen && $farm_cultivation_current_screen->id !== 'appearance_page_farm-cultivation-guide-page') {
            ?>

            <div class="farm-cultivation-admin-notice notice notice-info is-dismissible content-install-plugin theme-info-notice" id="farm-cultivation-dismiss-notice" data-notice="get_started_notice">
                <div class="notice-div">
                    <div>
                        <p class="theme-name"><?php echo esc_html($farm_cultivation_theme->get('Name')); ?></p>
                        <p><?php _e('For information and detailed instructions, check out our theme documentation.', 'farm-cultivation'); ?></p>
                    </div>
                    <div class="notice-buttons-box">
                        ...
                        <a class="button-primary theme-install" id="install-activate-button" href="#"><?php _e('Begin Installation', 'farm-cultivation'); ?></a>`
                    </div>
                </div>
            </div>
            <?php
...
add_action('admin_notices', 'farm_cultivation_custom_admin_notice');

// After switching theme, reset dismissed notice option
add_action('after_switch_theme', 'farm_cultivation_after_switch_theme');
function farm_cultivation_after_switch_theme() {
