...
<body <?php body_class(); ?>>
	...
	<a class="skip-link screen-reader-text" href="#primary"><?php esc_html_e( 'Skip to content', 'graceful-arch-blog' ); ?></a>	
	...
	<div id="site-container">
		...
		<?php
			if ( is_home() && !is_paged() ) {
			    ...
			    $graceful_featured_boxes_enabled = graceful_arch_blog_options('featured_boxes_show');
			    if ( $graceful_featured_boxes_enabled ) {
			        get_template_part('template-parts/sections/featured', 'boxes');
			    }

			    // Show Post Ticker if enabled
			    $graceful_post_ticker_enabled = graceful_arch_blog_options('post_ticker_show');
			    if ( $graceful_post_ticker_enabled ) {
			        get_template_part('template-parts/sections/arch', 'ticker');
			    }
