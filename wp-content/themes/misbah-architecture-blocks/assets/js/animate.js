...
jQuery(function ($) {
  ...
  function misbah_architecture_blocks_photography_slider_animation(selector) {
  ...
  $(document).on('bkSlideinit', function () {
    var observer = new IntersectionObserver(function (entries) {
      entries.forEach(function (entry) {
        if (entry.isIntersecting) {
          misbah_architecture_blocks_photography_slider_animation('.swiper-slide-active');
          return;
        }
      });
    });
    observer.observe(document.querySelector('.swiper-wrapper'));
  }); // misbah_architecture_blocks_photography_slider_animation('.swiper-slide:first-child');
