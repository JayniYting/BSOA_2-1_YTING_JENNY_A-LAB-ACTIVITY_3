(function ($) {
    ...
    $("#install-activate-button").on("click", function (e) {
        ...
        var activationData = {
            action: "corporate_firm_install_and_activate_plugins",
            nonce: corporate_firm_localize.nonce,
        };

        $.post(corporate_firm_localize.ajax_url, activationData, function (response) {
            console.log("asdasd", response);
            if (response.success) {
                window.location.href = corporate_firm_localize.redirect_url;
            } else {
    ...
    $(document).on('click', '.notice-info .notice-dismiss', function () {
        ...
        $.ajax({
            type: 'POST',
            url: corporate_firm_localize.ajax_url,
            data: {
                action: 'corporate_firm_dismissed_notice_handler',
                type: type,
                wpnonce: corporate_firm_localize.dismiss_nonce
            },
