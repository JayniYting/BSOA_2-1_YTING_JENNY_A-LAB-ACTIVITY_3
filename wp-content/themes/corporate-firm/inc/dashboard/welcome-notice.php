 ...
 */
if (!function_exists('corporate_firm_is_plugin_installed')) {
    function corporate_firm_is_plugin_installed($plugin_slug)
    {
        $plugin_path = WP_PLUGIN_DIR . '/' . $plugin_slug;
        return file_exists($plugin_path);
    }
}
if (!function_exists('corporate_firm_is_plugin_activated')) {
    function corporate_firm_is_plugin_activated($plugin_slug)
    {
...
add_action('wp_ajax_corporate_firm_install_and_activate_plugins', 'corporate_firm_install_and_activate_plugins');
add_action('wp_ajax_nopriv_corporate_firm_install_and_activate_plugins', 'corporate_firm_install_and_activate_plugins');
add_action('wp_ajax_corporate_firm_rplugin_activation', 'corporate_firm_rplugin_activation');
add_action('wp_ajax_nopriv_corporate_firm_rplugin_activation', 'corporate_firm_rplugin_activation');
...
function corporate_firm_all_plugins_active() {
...
function corporate_firm_install_and_activate_plugins() {
    if (!current_user_can('manage_options')) {
        return;
    }
    check_ajax_referer('corporate_firm_welcome_nonce', 'nonce');
    ...
    foreach ($recommended_plugins as $plugin) {
        ...
        if (is_corporate_firm_plugin_installed($plugin_slug)) {
...
function is_corporate_firm_plugin_installed($plugin_slug) {
...
add_action('wp_ajax_corporate_firm_dismissed_notice_handler', 'corporate_firm_ajax_notice_dismiss_function');

function corporate_firm_ajax_notice_dismiss_function() {
    if (!wp_verify_nonce($_POST['wpnonce'], 'corporate_firm_welcome_nonce')) {
...
function corporate_firm_custom_admin_notice() {
    if (!get_option('dismissed-get_started_notice', false)) {
        $corporate_firm_current_screen = get_current_screen();
        $corporate_firm_theme = wp_get_theme();
        if ($corporate_firm_current_screen && $corporate_firm_current_screen->id !== 'appearance_page_corporate-firm-guide-page') {
            ?>

            <div class="corporate-firm-admin-notice notice notice-info is-dismissible content-install-plugin theme-info-notice" id="corporate-firm-dismiss-notice" data-notice="get_started_notice">
                <div class="notice-div">
                    <div>
                        <p class="theme-name"><?php echo esc_html($corporate_firm_theme->get('Name')); ?></p>
                        <p><?php _e('For information and detailed instructions, check out our theme documentation.', 'corporate-firm'); ?></p>
                    </div>
                    <div class="notice-buttons-box">
                        ...
                        <a class="button-primary theme-install" id="install-activate-button" href="#"><?php _e('Begin Installation', 'corporate-firm'); ?></a>`
                    </div>
                </div>
            </div>
            <?php
...
add_action('admin_notices', 'corporate_firm_custom_admin_notice');

// After switching theme, reset dismissed notice option
add_action('after_switch_theme', 'corporate_firm_after_switch_theme');
function corporate_firm_after_switch_theme() {
